...
void hideSystemCursor() {
    ...
    unsigned char ANDmaskCursor[] = { 0xFF };
    unsigned char XORmaskCursor[] = { 0x00 };

    HCURSOR hBlank = CreateCursor(GetModuleHandle(NULL), 0, 0, 1, 1, ANDmaskCursor, XORmaskCursor);
    SetClassLong(GetForegroundWindow(), GCL_HCURSOR, (LONG)hBlank);
    SetCursor(hBlank);
}
// ---------- GAME VARIABLES ---------- //
// Bricks layout
...
void playBrickSound() {
    PlaySound("brick.wav", NULL, SND_ASYNC | SND_NOSTOP);
}
void playLifeLost() {
    PlaySound("life.wav", NULL, SND_ASYNC | SND_NOSTOP);
}
void playServe() {
    PlaySound("serve.wav", NULL, SND_ASYNC | SND_NOSTOP);
}
// stop all
void stopSound() {
...
int main() {
    // Fullscreen window
    initwindow(GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), "Brick Breaker", -3, -3);
hideSystemCursor();   // yeh original system cursor ko transparent kar dega
    ...
    closegraph();
    stopSound();
    SetClassLong(GetForegroundWindow(), GCL_HCURSOR, (LONG)LoadCursor(NULL, IDC_ARROW));
